...
def get_db_connection():
    ...
    # Add password if provided (blank passwords work fine)
    db_password = os.getenv('DB_PASSWORD')
    if db_password:  # Only add password if it's not empty
...
def hash_datapoint(company_id, period_id, line_item_id, value, source_file):
    """Generate a unique hash for a data point to avoid duplicates"""
    hash_string = f"{company_id}_{period_id}_{line_item_id}_{value}_{source_file}"
